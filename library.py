...
def clear_fields():
    global bk_status, bk_id, bk_name, author_name, card_id, student_name, student_id, student_email

    bk_status.set('Available')
    for i in ['bk_id', 'bk_name', 'author_name', 'card_id', 'student_name', 'student_id', 'student_email']:
        exec(f"{i}.set('')")
...
def view_record():
    global bk_name, bk_id, bk_status, author_name, card_id, student_name, student_id, student_email
    global tree
    ...
    student_name.set(selection[5])
    student_id.set(selection[6])
    student_email.set(selection[7])

def add_record():
    global connector
    global bk_name, bk_id, author_name, bk_status, card_id, student_name, student_id, student_email
    ...
    if surety:
        try:
            connector.execute(
                'INSERT INTO Library (BK_NAME, BK_ID, AUTHOR_NAME, BK_STATUS, CARD_ID, STUDENT_NAME, STUDENT_ID, STUDENT_EMAIL) VALUES (?, ?, ?, ?, ?, ?, ?, ?)',
                (bk_name.get(), bk_id.get(), author_name.get(), bk_status.get(), card_id.get(), student_name.get(), student_id.get(), student_email.get()))
            connector.commit()
            clear_and_display()
...
def update_record():
    def update():
        global bk_status, bk_name, bk_id, author_name, card_id, student_name, student_id, student_email
        ...
        cursor.execute('UPDATE Library SET BK_NAME=?, BK_STATUS=?, AUTHOR_NAME=?, CARD_ID=?, STUDENT_NAME=?, STUDENT_ID=?, STUDENT_EMAIL=? WHERE BK_ID=?',
                       (bk_name.get(), bk_status.get(), author_name.get(), card_id.get(), student_name.get(), student_id.get(), student_email.get(), bk_id.get()))
        connector.commit()
...
Label(left_frame, text='Student Name', bg=lf_bg, font=lbl_font).place(x=85, y=345)
Entry(left_frame, width=25, font=entry_font, text=student_name).place(x=45, y=375)

Label(left_frame, text='Student ID', bg=lf_bg, font=lbl_font).place(x=105, y=425)
student_id_entry = Entry(left_frame, width=25, font=entry_font, text=student_id)
student_id_entry.place(x=45, y=455)

Label(left_frame, text='Student Email', bg=lf_bg, font=lbl_font).place(x=85, y=505)
Entry(left_frame, width=25, font=entry_font, text=student_email).place(x=45, y=535)

submit = Button(left_frame, text='Add new record', font=btn_font, bg=btn_hlb_bg, width=20, command=add_record)
submit.place(x=50, y=600)

clear = Button(left_frame, text='Clear fields', font=btn_font, bg=btn_hlb_bg, width=20, command=clear_fields)
clear.place(x=50, y=660)
...
tree = ttk.Treeview(RB_frame, selectmode=BROWSE, columns=('Book Name', 'Book ID', 'Author', 'Status', 'Issuer Card ID', 'Student Name', 'Student ID', 'Student Email'))
...
tree.heading('Student Email', text='Student Email', anchor=CENTER)
...
tree.column('#7', width=200, stretch=NO)
